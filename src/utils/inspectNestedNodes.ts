...
// export function inspectNestedObjectsOrArraysNodelike<
export function inspectNestedNodes<T extends InspectableDataProps>(
  ...
  if (Array.isArray(data)) {
    ...
    data.forEach((item) => {
      // result += inspectNestedObjectsOrArraysNodelike(item, indentLevel + 1);
    ...
    Object.entries(data).forEach(([key, value], index) => {
      ...
      if (typeof value === 'object' && value !== null) {
        // result += inspectNestedObjectsOrArraysNodelike(
